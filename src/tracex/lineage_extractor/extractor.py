...
class DbtColumnLineageExtractor:
    def __init__(self, manifest_path, catalog_path, selected_models=[]):
        # Set up logging
        self.logger = logging.getLogger("tracex")
        ...
        self.tracex_version = self._get_tracex_version()
    ...
    def _get_tracex_version(self):
        try:
            return version("tracex")
...
class DBTNodeCatalog:
    def __init__(self, node_data):
        ...
        # Handle missing unique_id gracefully
        # unique_id may not be present in all dbt versions or edge cases
        if "unique_id" in node_data:
            self.unique_id = node_data["unique_id"].lower()
        else:
            # Fallback: construct a unique identifier from available metadata
            # Format: {database}.{schema}.{name} (lowercased for consistency)
            db = metadata.get("database", "unknown")
            schema = metadata.get("schema", "unknown")
            name = metadata.get("name", "unknown")
            self.unique_id = f"{db}.{schema}.{name}".lower()
